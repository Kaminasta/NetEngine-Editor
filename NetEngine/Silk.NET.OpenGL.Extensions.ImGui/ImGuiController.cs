...
public class ImGuiController : IDisposable
{
    ...
    /// </summary>
    public ImGuiController(GL gl, IView view, IInputContext input, ImGuiFontAtlasConfig imGuiFontConfig) : this(gl, view, input, imGuiFontConfig, null)
    {
    }
    ...
    /// </summary>
    public ImGuiController(GL gl, IView view, IInputContext input, ImGuiFontAtlasConfig? imGuiFontConfig = null, Action onConfigureIO = null)
    {
        Init(gl, view, input);
        ...
        {
            for (int i = 0; i < imGuiFontConfig.Fonts.Count; i++)
            {
                var fontConfig = imGuiFontConfig.Fonts[i];

                var glyphRange = fontConfig.GetGlyphRange?.Invoke(io) ?? default;
                var config = fontConfig.FontConfig ?? null;

                io.Fonts.AddFontFromFileTTF(fontConfig.FontPath, fontConfig.FontSize, config, glyphRange);
            }
        }
    ...
    private void CreateDeviceResources()
    {
        ...
        _shader = new(_gl, vertexSource, fragmentSource);
    ...
    private unsafe void RecreateFontDeviceTexture()
    {
        ...
        _fontTexture = new(_gl, width, height, pixels);
        _fontTexture.Bind();
